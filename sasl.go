...
// SASLMechanism implements the SASL state machine.  It is initialized by
// calling Start and sending the initial bytes to the server.  The protocol then
// loops by passing the server's response into Next and then sending Next's
// bytes to the server.  Eventually either Next will indicate that the
// authentication has been successfully completed or an error will cause the
// state machine to exit prematurely.
//
// A SASLMechanism must be re-usable, but it does not need to be safe for
// concurrent access by multiple go routines.
type SASLMechanism interface {
	...
	//
	// In order to ensure that the SASLMechanism is reusable, calling Start must
