...
// Start and sending the initial bytes to the server.  The protocol then loops
// by passing the server's response into Next and then sending Next's bytes to
// the server.  Eventually either Next will indicate that the authentication
// has been successfully completed or an error will cause the auth to end
// prematurely.
//
// A Mechanism must be re-usable, but it does not to be safe for concurrent
// access by multiple go routines.
type Mechanism interface {
	// Begins SASL authentication. It returns the authentication mechanism name
	// and "initial response" data (if required by the selected mechanism). A
	// non-nil error causes the client to abort the authentication attempt.
	//
